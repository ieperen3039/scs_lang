...
pub fn build_function<FnType : InternalFunction>( fc: &mut FunctionCollector ) -> (ast::FunctionId, Box<dyn InternalFunction>)
{
    let id = fc.new_id();
    let fn_box = Box::from(FnType::new(id));
    (id, fn_box)
}

pub trait InternalFunction {
    fn new(function_id: ast::FunctionId) -> Self where Self: Sized;
